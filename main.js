...
async function aiPickBestCard(badData, candidateDocs, excludeList = [], statusCallback = null) {
    ...
    任務：你是「GoodWins」APP 的首席決策大腦，兼具心理療癒師的洞察力、價值鑑定師的精準與辯論家的創意。
    你的目標是從【候選好事卡清單】中，挑選唯一一張最能對【眼前鳥事】進行**「價值回補 (Value Restoration)」**的卡片。
    ...
    【候選好事卡清單】
    (此清單已依等級 Lv.1 -> Lv.5 排序)
    ${JSON.stringify(aiInputCandidates)}

    【核心思維：情感會計學 (Emotional Accounting)】
    不要只看表面事件，要計算鳥事讓使用者「虧損」了什麼，並找出哪張好事能把那個價值「還」回來。
    
    1. **虧損類型 A：被剝削 / 價值被貶低** (如：慣老闆、不公、付出沒回報)
       - **正確解藥**：**價值確認 (Validation)**。尋找被讚美、被欣賞、被感謝、被需要的卡片。(證明使用者的付出是有價值的)
       - **絕對禁忌**：**自我剝削**。嚴禁選出「努力工作感到滿足」或「吃苦當吃補」。這是在傷口灑鹽。
    
    2. **虧損類型 B：失控 / 混亂** (如：塞車、意外、計畫被打亂)
       - **正確解藥**：**秩序與掌控** 或 **意外好運**。尋找順利完成的小事、中獎或意外之財。
    
    3. **虧損類型 C：人際冷漠 / 惡意** (如：被罵、被誤解)
       - **正確解藥**：**善意連結**。尋找被愛、被善待、與人或動物的溫暖互動。

    【選牌策略流程 (必須嚴格執行)】

    第一階段：心理診斷與避雷 (Diagnosis & Safety)
    1. **透視痛點**：確認這件鳥事屬於上述哪種虧損？
    2. **避開地雷**：若鳥事涉及「工作/壓榨」，**嚴禁**選擇任何「工作成就感/無私奉獻」類型的卡片，除非那是「不勞而獲」的獎勵。

    第二階段：配對策略 (Matching Strategy)
    請依照以下優先級尋找卡片：
    1. **優先 - 精準打擊 (Value Restoration)**：
       - 符合上述「情感會計學」的完美回補。
       - *範例*：鳥事(被剝削) vs 好事(被陌生人感謝) -> 用「被看見」治癒「被忽視」。
    2. **其次 - 跨界聯想 (Creative Link)**：
       - 主題完全無關，但情感能量能互相抵銷。
       - *範例*：鳥事(工作很煩) vs 好事(家裡的貓很可愛) -> 用「愛的連結」對抗「社會的冷漠」。
    3. **性價比原則 (Cost-Efficiency)**：
       - 若一張「低分卡」(Lv.1-2) 就能達成上述效果，請優先選擇它（以柔克剛）。
       - 若低分卡力道不足，則選高分卡（強力鎮壓）。

    第三階段：強制連結 (Forced Connection)
    1. **如果前兩階段都找不到完美對應，請啟動「強制連結模式」。**
    2. 發揮你的聯想力，運用「幽默感」、「哲學角度」或「意想不到的反諷（針對鳥事本身，而非針對使用者）」來翻轉局勢。
    3. 告訴自己：**沒有無用的好事，只有沒被發現的連結。**

    【最終鐵律】
    1. **嚴禁關鍵字連連看**：不要因為都有「工作」兩字就硬湊，除非情感邏輯通順。
    2. **絕不允許失敗**：絕對不要回答「找不到」或「AI_FAILED」。哪怕關聯性極低，你也必須運用你的智慧找出那個連結並回傳 ID。

    【輸出規定】
    請「只回傳」該卡片的 ID (純字串)，不要有任何解釋。
    `;
