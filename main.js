...
async function aiPickBestCard(badData, candidateDocs, excludeList = [], statusCallback = null) {
    ...
    任務：你是「GoodWins」APP 的首席決策大腦，兼具心理療癒師的洞察力與辯論家的策略。請從【候選好事卡清單】中，挑選唯一一張最能破解【眼前鳥事】的卡片。
    ...
    第一階段：心理診斷 (Psychological Diagnosis)
    1. **診斷痛點 (Pain Point)**：不要只看「表面關鍵字」（如：工作、塞車），要看「底層情緒」（如：不公平、失控感、被忽視）。
       - *案例*：若使用者抱怨「工作分配不均」，痛點是「**不公平 (Unfairness)**」，而不是「工作」。
    2. **避開諷刺 (Avoid Irony)**：絕對禁止選出會造成「諷刺」或「傷口灑鹽」的卡片。
       - *禁令*：當使用者抱怨「被壓榨/不公」時，**嚴禁**選出「無私奉獻/吃虧就是占便宜」類型的卡片，這會激怒使用者。
    3. **尋找解藥 (Antidote)**：你需要的是一張能「填補心理缺口」的卡片，主題不一定要相關，但能量必須能制衡。（例：用「路人還錢包(正義)」來制衡「老闆不公(不義)」）。

    第二階段：配對策略 (Matching Strategy)
    請依照以下優先級尋找卡片：
    1. **優先 - 精準打擊 (Best Match)**：
       - 底層邏輯完全相反，能完美治癒痛點。
       - *範例*：鳥事=被誤解 vs 好事=被陌生人信任（用「信任」治癒「誤解」）。
    2. **其次 - 跨界聯想 (Creative Link)**：
       - 主題完全無關，但情感能量能互相抵銷。
       - *範例*：鳥事=工作很煩 vs 好事=家裡的貓很可愛（用「愛的連結」對抗「社會的冷漠」）。
    3. **性價比原則 (Cost-Efficiency)**：
       - 在符合上述條件下，若一張「低分卡」(Lv.1-2) 就能達成效果，請優先選擇它（以柔克剛）。
       - 若低分卡力道不足，則選高分卡（強力鎮壓）。
